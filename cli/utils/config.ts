 ...
 */
export async function readConfig(configPath: string): Promise<ClafoutisConfig | null> {
  try {
 ...
 * Returns null if the file does not exist or cannot be parsed.
 */
export async function readProducerConfig(configPath: string): Promise<ProducerConfig | null> {
  try {
    const content = await fs.readFile(configPath, 'utf-8');
