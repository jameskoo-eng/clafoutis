...
describe('Producer E2E', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'clafoutis-e2e-producer-'));
  });
  ...
  it('init --producer creates config, tokens, and workflow', async () => {
    ...
    const configExists = await fileExists(path.join(tempDir, '.clafoutis/producer.json'));
    const tokensExist = await fileExists(path.join(tempDir, 'tokens/colors/primitives.json'));
    const workflowExists = await fileExists(path.join(tempDir, '.github/workflows/clafoutis-release.yml'));
  ...
  it('generate --dry-run does not create files', async () => {
    ...
    execSync(`node ${cliBin} generate --dry-run`, { cwd: tempDir, stdio: 'pipe' });
